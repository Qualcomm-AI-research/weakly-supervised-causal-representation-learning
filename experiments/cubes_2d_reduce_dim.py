...
from ws_crl.encoder.vae import GaussianEncoder
...
@hydra.main(config_path="../config", config_name="2dcubes_reduce_dim")
def main(cfg):
...
def create_encoder_decoder(cfg):
    ...
    elif cfg.model.encoder.type == "mlp":
        dim_x_flat = np.prod(cfg.model.dim_x)

        encoder_hidden_layers = cfg.model.encoder.hidden_layers
        if isinstance(cfg.model.encoder.hidden_units, list):
            encoder_hidden = cfg.model.encoder.hidden_units
        else:
            encoder_hidden = [dim_x_flat // 2 ** (i + 5) for i in range(encoder_hidden_layers)]
            # encoder_hidden = [cfg.model.encoder.hidden_units for _ in range(encoder_hidden_layers)]

        logger.info(f"Encoder hidden units: {encoder_hidden}")

        encoder = GaussianEncoder(
            hidden=encoder_hidden,
            input_features=dim_x_flat,
    ...
    elif cfg.model.decoder.type == "mlp":
        dim_x_flat = np.prod(cfg.model.dim_x)

        decoder_hidden_layers = cfg.model.decoder.hidden_layers
        if isinstance(cfg.model.decoder.hidden_units, list):
            decoder_hidden = cfg.model.decoder.hidden_units
        else:
            decoder_hidden = encoder_hidden[::-1] + [dim_x_flat]
            # decoder_hidden = [cfg.model.decoder.hidden_units for _ in range(decoder_hidden_layers)]

        logger.info(f"Decoder hidden units: {decoder_hidden}")
        decoder = GaussianEncoder(
            hidden=decoder_hidden,
            input_features=cfg.model.dim_z,
            output_features=dim_x_flat,
            fix_std=cfg.model.decoder.fix_std,
