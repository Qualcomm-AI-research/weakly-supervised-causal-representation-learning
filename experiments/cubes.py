...
@hydra.main(config_path="../config", config_name="cubes_64")
def main(cfg):
...
def get_dataloader(cfg, tag, batchsize=None, shuffle=False, include_noise_encodings=False):
    ...
    dataset = SceneCausalImageDataset(
...
# TODO: add intervention masks for non-atomic interventions
# TODO: don't use nested dataset
class SceneCausalImageDataset(Dataset):
    def __init__(
        self,
        base_dir,
        tag,
        ...
        intervention_directories = next(os.walk(tag_dir))[1]

        x0 = []
        x1s = defaultdict(list)

        z0 = []
        z1s = defaultdict(list)

        e0 = []
        e1s = defaultdict(list)

        # intervention_labels = []

        # read images
        for directory in intervention_directories:
            data_directory = Path(tag_dir) / directory / "data"
            files = next(os.walk(data_directory))[2]
            files = sorted(files)
            for file in files:
                if not file.endswith(".jpg") and not file.endswith(".png"):
                    continue
                img = Image.open(data_directory / file)
                img = np.array(img)
                if directory == original_image_dir:
                    x0.append(img)
                else:
                    x1s[directory].append(img)
                    # intervention_labels.append(int(directory))

        # read labels
        for directory in intervention_directories:
            labels_directory = Path(tag_dir) / directory / "labels"
            files = next(os.walk(labels_directory))[2]
            files = sorted(files)
            for file in files:
                if not file.endswith(".json"):
                    continue
                with open(labels_directory / file) as f:
                    data = json.load(f)
                scm_outcomes = data["scm_outcomes"]
                # Sort the keys to ensure the same order
                scm_outcomes = {k: scm_outcomes[k] for k in sorted(scm_outcomes.keys())}
                # Select the z axis of all the position vectors
                z = [v[2] for k, v in scm_outcomes.items() if k.startswith("pos_")]
                assert len(z) == 3

                scm_noise_values = data["scm_noise_values"]
                scm_noise_values = {k: scm_noise_values[k] for k in sorted(scm_noise_values.keys())}
                e = [v for k, v in scm_noise_values.items() if k.startswith("pos_")]
                if directory == original_image_dir:
                    z0.append(z)
                    e0.append(e)
                else:
                    z1s[directory].append(z)
                    e1s[directory].append(e)

        self._x0 = np.array(x0, dtype=np.float32)
        self._x1s = {int(k): np.array(v, dtype=np.float32) for k, v in x1s.items()}
        self._z0 = np.array(z0, dtype=np.float32)
        self._z1s = {int(k): np.array(v, dtype=np.float32) for k, v in z1s.items()}
        self._e0 = np.array(e0, dtype=np.float32)
        self._e1s = {int(k): np.array(v, dtype=np.float32) for k, v in e1s.items()}
        # self._intervention_labels = np.array(intervention_labels, dtype=np.float32)[np.newaxis, :].T

        # check if x1s have same shape as x0
        for k, v in self._x1s.items():
            assert v.shape == self._x0.shape, f"x shape mismatch for {k}"

        # check if z1s have same shape as z0
        for k, v in self._z1s.items():
            assert v.shape == self._z0.shape, f"z shape mismatch for {k}"

        self._num_interventions = len(self._z1s)

        self.noise = noise

    def __getitem__(self, index):
        partition_length = self._z0.shape[0]
        _index = index % partition_length
        intervention_index = int(index / partition_length)
        x0 = torch.FloatTensor(self._x0[_index]).permute(2, 0, 1) / 255
        if intervention_index == 0:
            x1 = x0
        else:
            x1 = torch.FloatTensor(self._x1s[intervention_index][_index]).permute(2, 0, 1) / 255

        z0 = torch.FloatTensor(self._z0[_index])
        if intervention_index == 0:
            z1 = z0
        else:
            z1 = torch.FloatTensor(self._z1s[intervention_index][_index])

        intervention_label = torch.LongTensor([intervention_index])
        ...
        e0 = torch.FloatTensor(self._e0[_index])
        if intervention_index == 0:
            e1 = e0
        else:
            e1 = torch.FloatTensor(self._e1s[intervention_index][_index])
        return x0, x1, z0, z1, intervention_label, e0, e1

    def __len__(self):
        return self._z0.shape[0] * (self._num_interventions + 1)
